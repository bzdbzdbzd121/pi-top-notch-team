...
- **📨 实时消息通道** — Agent 之间通过内置消息总线通信（无需外部基础设施），成员互发可一键收紧为「仅限回复 TL」
...
TL 还能自主开会：当它判断某个任务适合整队委派时（例如「帮我重构这个模块」），可直接调用 `start_team_session` 现场组建动态团队，全程自主——设计团队、派发任务、汇总结果，收尾用 `stop_team_session` 结束（会话目录同样保留，可 `/team resume` 恢复）。

与 `/team dynamic` 的区别是不做需求追问、不设计划确认门，直接开工；也因此不受团队会话的写入守卫限制，可自由读写文件。状态栏会以 🤖 标记这类会话。不想要这种行为时，在 `/team setting` 把「Agent 自主团队会话」设为「禁止」（运行中的会话不受影响）。
...
交互式设置菜单（会话内外均可使用），两种作用域：

- **仅当前会话（临时，默认）**——不写全局 `settings.yaml`；设置随本团队会话存快照，`/team resume` 时自动还原（无法获取会话标识时回退全局）
- **全局**——写入 `~/.pi/top-notch-team/settings.yaml`，对所有新会话生效

| 设置 | 说明 |
|------|------|
| 成员默认模型 | `跟随 TL 当前模型` 或 `固定为某个已登录模型`；优先级：成员 YAML > 团队 YAML 默认 > 此处。只影响之后启动的成员 |
| 成员思考强度 | `默认` / `跟随 TL` / `指定级别`；模型不支持该级别时保持默认。只影响之后启动的成员 |
| 自动压缩 | 成员空闲且即将收到新任务时，上下文超阈值（百分比和/或绝对 token 数）先压缩再派发；可配置开关、阈值与压缩超时（默认 10 分钟） |
| 等待上限 | `team_send_and_wait` 的 all-idle 门控与批屏障共用（默认 15 分钟，`0` = 永不超时） |
| 消息合并 | 成员积压消息在接收方回合结束时合并为单条派发，减少回合碎片（开关 / 条数 / 字符上限） |
| 成员互发消息 | `允许` / `仅限回复 TL`；拦截即时生效，成员侧工具面与提示词只影响之后启动的成员 |
| Agent 自主团队会话 | `允许` / `禁止` agent 用 `start_team_session` 自主开会；下一回合边界生效，运行中的会话不受影响 |

临时作用域改动的项会标注 `[临时]`，菜单另有一键**清除全部临时设置**（恢复全局生效）。
...
| `/team setting` | 团队设置（临时/全局）——模型、思考强度、自动压缩、等待上限、消息合并、成员互发、agent 自主会话 |
| `/team help` | 显示使用帮助 |
...
此外 `start_team_session(task)` 在扩展加载时即注册（agent 自主开会的入口），`stop_team_session()` 仅在自主会话中激活。
