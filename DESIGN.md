...
Router (processes one message at a time)
  ...
  │                     入 tlWaitGate 扩展侧缓冲，门控打开后并入
  │                     team_send_and_wait 工具结果一并返回（[from message] 段落）
...
**S3 amendment (阶段 3, 决策 #39 — wait-gate buffered, returned INSIDE the tool result)**: while a `team_send_and_wait` wait is in flight, non-reply member→TL messages buffer in the extension-side `tlWaitGate` (`src/channel/tl-wait-gate.ts`) instead of pi's nextTurn queue (which has no public drain API — the buffering decision must be made at message-arrival time). When the mandatory all-idle gate opens (decision #38), `waitWithAllIdleCheck` drains the gate and appends each buffered message to the tool result as an `[from message]` section (subject included, no meta headers) after the reply parts and before the nextSteps footer — the TL reads replies and wait-period messages in ONE history entry, zero separate injections. Rationale (user ruling): the mandatory all-idle gate spans the members' entire working window, so non-reply messages can only arrive while the gate is active — there is nothing to deliver outside a wait. Esc-abort safe (verified in agent-loop.js `executeToolCallsSequential`): a completed tool result is pushed into context BEFORE the abort-break, so the result (buffered messages included) survives to the next turn. Design history: v1 delivered via a plain `pi.sendMessage` without `deliverAs` (steer branch — during tool execution the agent run is active and the steering queue drains after tool results / before the next completion; verified against the real AgentSession, then removed in v2 in favor of the simpler in-result return). No-wait periods (late post-settlement messages, Inspector-driven replies) keep pure S2 nextTurn semantics; the batch-barrier window (before `beginWait`) also stays S2.
