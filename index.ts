...
### 与用户讨论需求的方式（参考 /grill-with-docs 方法）
在拆解任务之前，先与用户充分讨论，逐条确认，直到达成共识：

1. **逐问确认** — 一次只问一个问题，等用户回复后再问下一个。不要一次性抛出多个问题。
2. **挑战模糊语言** — 当用户用词不精确时，提出更精确的术语。例如用户说"优化性能"——追问"你指的是减少响应时间还是降低资源占用？"
3. **用场景验证** — 提出具体的边界场景来检验需求。例如"如果 A 成员依赖 B 成员的结果，但 B 还没完成怎么办？"
4. **对照代码验证** — 当用户描述现有行为时，检查代码是否一致。矛盾时指出来让用户确认。
5. **更新共享上下文** — 讨论中确定的关键术语、决策、约定，立即写入 .shared-context.md。不要攒到后面才写。
