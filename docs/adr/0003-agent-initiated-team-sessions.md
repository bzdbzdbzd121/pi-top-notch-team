...
- The entry is policy-gated by a global setting (决策 #42, `allowAgentInitiatedSessions`, default `true` = unchanged behavior): when disabled, the tool is removed from the active tool set at every turn boundary (its prompt surface vanishes with it) and its execute rejects with zero side effects. Disabling only blocks NEW starts — running sessions are not terminated; lifecycle (`stop_team_session`, `/team resume`) and in-session guards are unchanged. A three-state ("confirm" gate) evolution is reserved as a documented note: if ever needed, add a new key + migration (waitTimeoutMinutes precedent); the boolean key does not weld the semantics.
