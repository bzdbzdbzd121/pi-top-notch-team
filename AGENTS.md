...
| `/team resume [标识]` | 恢复中断或已停止的团队会话：按 session.json 清单重建状态，成员以 `--continue` 带完整上下文重启（ADR-0004） |
